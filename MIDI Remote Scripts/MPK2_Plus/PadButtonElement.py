...
class PadButtonElement(ButtonElement):
	...
	def __init__(self, is_momentary, msg_type, channel, identifier, pad_num = 0, skin = Skin(), undo_step_handler = DummyUndoStepHandler(), *a, **k):
		...
		self.send_midi( (Sysex.SYX_START, ) + (Sysex.SYX_AKAI_ID) + (Sysex.SYX_DEVICE_ID_MPK249, ) + (Sysex.SYX_FUNC_SET_PAD_COLOR) + (Sysex.SYX_PAD_ON_BASE_HB, ) + ( Sysex.SYX_PAD_ON_BASE_LB + self._pad_num, ) + (0, ) + (Sysex.SYX_END, ) )
	...
	def send_value(self, value, force = False, channel = None): 
		...
		if (int_value != self._old_value):      # don't fill up the MIDI bus if the color hasn't changed
			...
			if (int_value != 0):
				...
				self.send_midi( (Sysex.SYX_START, ) + (Sysex.SYX_AKAI_ID) + (Sysex.SYX_DEVICE_ID_MPK249, ) + (Sysex.SYX_FUNC_SET_PAD_COLOR) + (pad_off_hb, ) + ( pad_off_lb, ) + (int_value, ) + (Sysex.SYX_END, ) )
				...
				self.send_midi( (Sysex.SYX_START, ) + (Sysex.SYX_AKAI_ID) + (Sysex.SYX_DEVICE_ID_MPK249, ) + (Sysex.SYX_FUNC_SET_PAD_COLOR) + (pad_off_hb, ) + ( pad_off_lb, ) + (0, ) + (Sysex.SYX_END, ) )
